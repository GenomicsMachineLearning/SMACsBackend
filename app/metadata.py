...
# Paths
# 1. Try QRIS Path (Existing Server)
QRIS_PATH = "/QRISdata/Q5058/sMACS/Data_for_SMACsApp/"

# 2. Try Relative App Path (AWS / GitHub Deployment)
AWS_PATH = os.path.join(os.path.dirname(__file__), "data")
...
# Set Subdirectories
LR_DIR = os.path.join(DATA_DIR, "LRInteractions")
...
class MetadataService:
    ...
    def _load_sample_config():
        try:
            config_path = os.path.join(os.path.dirname(__file__), "sample_metadata.json")
    ...
    @staticmethod
    def get_sample_files(organ: str, technology: str, age_group: str, mode: str = "Ligand-Receptor"):
        """
    ...
    @staticmethod
    def calculate_stats(organ: str, age_group: str):
        """
        Reads [Organ]_Proportions.csv, filters by Tech/Age, calculates mean cell type props.
        Returns formatted string.
        """
        pass # Not used currently in favor of get_stats_for_group
    ...
    def get_stats_for_group(organ: str, technology: str, age_group: str):
        ...
        try:
            ...
            if subset.empty:
                print(f"Stats Debug: Filter failed for {target_tech}/{target_age} in {csv_path}")
            ...
            numeric_cols = subset.select_dtypes(include=['float64', 'float32', 'int']).columns
